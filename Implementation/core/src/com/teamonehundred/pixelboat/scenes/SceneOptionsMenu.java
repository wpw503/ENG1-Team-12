...
/**
 * Represents the Options Menu Scene for when the player wants to select/edit
   the options before the race starts.
 *
...
public class SceneOptionsMenu implements Scene {
  ...
  * When options are hovered over they will change texture indicating what the user is about
    to select.
  *
  * @author William Walton
  */
  public SceneOptionsMenu() {
    ...
    backHovered = new Texture("options_menu_backHovered.png");
  ...
  */
  ...
  /**
  * Update function for SceneOptionsMenu. Ends SceneOptionsMenu based on user input otherwise 
    stays in scene.
  * 
  * <p>Returns an specified integer when you want to exit the screen else return sceneID if 
    you want to stay in scene.
  ...
  */
  public int update() {
    ...
    if (fullCheckSprite.getBoundingRectangle().contains(mousePos.x, mousePos.y)) {
      //fullCheckSprite.setTexture(full_check_);
      if (Gdx.input.isButtonPressed(Input.Buttons.LEFT)) {
        if (isFullscreen) {
