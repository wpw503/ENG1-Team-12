...
/**
 * Scene Class for Results Screen. Contains all functionality for Displaying results
   screen after each leg.
 *
 * @author Umer Fakher
...
public class SceneResultsScreen implements Scene {
  ...
  /**
  * Main constructor for a SceneResultsScreen.
  * 
  * <p>Initialises a Scene textures for the results screen and camera.
  * 
  *
  * @author ---
  */
  ...
  /**
   * Update function for SceneResultsScreen. Ends SceneResultsScreen based on user input
     otherwise stays in scene.
   * 
   * <p>Returns 1 when you want to exit the results screen else return sceneID if you
     want to stay in scene.
   *
   * @return returns an integer which is either the sceneID or 1
   * @author Umer Fakher
   */
  public int update() {
   ...
   * 
   * <p>Draws ResultsScreen which includes the leg time for all boats (AI and player boats) that
   * have just completed a leg. This table will wrap according to how many boat times need to be
   * displayed. Using label template format it draws the name of boat, time of just completed 
   * leg, race penalty added for each boat that finished the leg.
   *
  ...
  public void draw(SpriteBatch batch) {
    ...
    for (Boat b : boats) {
      ...
      // Draw to results display to screen using position of player's UI and draw for all 
      // boats this down the and wraps across screen if needed into the next column
      font.draw(batch, labelText, -thePlayerBoat.ui_bar_width / 2 + columnNum * 210,
