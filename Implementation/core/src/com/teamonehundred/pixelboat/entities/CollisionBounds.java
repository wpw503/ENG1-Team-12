 ...
 * Helps to control functionality for objects colliding by defining bounds that are used for
...
public class CollisionBounds {
  ...
  */
  public List<Polygon> getPolygons() {
    ...
    for (Shape2D myBound : bounds) {
      ret.add(getPolygon((Rectangle) myBound, rotation, origin));
    }
    return ret;
  }
  ...
  public boolean isColliding(CollisionBounds collider) {
    for (Shape2D myBound : bounds) {
      for (Shape2D theirBound : collider.getShapes()) {
        if (myBound instanceof Rectangle && theirBound instanceof Rectangle) {
          if (rectOnRectCollides((Rectangle) myBound, rotation, origin, (Rectangle) theirBound,
              collider.getRotation(), collider.getOrigin())) {
            return true;
          }
        }
      }
  ...
  * @return boolean
  */
  public boolean rectOnRectCollides(Rectangle r1, float rot1, Vector2 o1, Rectangle r2,
      float rot2, Vector2 o2) {
