...
public class AIBoat extends Boat {
  ...
  * @param w                  int width of the new boat
  * @param h                  int height of the new boat
  * @param texturePath       String relative path from core/assets folder of the boats texture image
  ...
  * @author James Frost
  */
  AIBoat(int x, int y, int w, int h, String texturePath, String name, float durabilityPerHit,
      float staminaUsage, float staminaRegen) {
    super(x, y, w, h, texturePath, name, durabilityPerHit, staminaUsage, staminaRegen); 
    // This should be the init that is used mostly (but the other one is needed incase of problems)
    initialise();
  }

  /**
  * Shared initialisation functionality among all constructors.
  * Initialises the ray properties. 
  * Rays are used to help the AI control the boat based on visual feedback
  ...
  */
  public void initialise() {
    ...
    rayRange = 30; // the range of each ray
  ...
  * @author James Frost
  */
  protected Vector2 getRayFirePoint() {
  ...
  */
  protected void check_turn(List<CollisionObject> collidables) {
    ...
    for (int ray = 0; ray <= numberOfRays; ray++) {
      ...
      for (float dist = 0; dist <= rayRange; dist += rayStepSize) {
